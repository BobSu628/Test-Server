...
/**
 * A server for a network multi-player tic tac toe client.client_game.  Modified and
 ...
 * of the code.  The client.client_game change is instead of passing *data* between the
 * client and server, I made a TTTP (tic tac toe protocol) which is totally
 * plain text, so you can test the client.client_game with Telnet (always a good idea.)
...
/**
 * A two-player client.client_game.
 */
class Game {
     ...
     * be occupied.  If the move is legal the client.client_game state is updated
